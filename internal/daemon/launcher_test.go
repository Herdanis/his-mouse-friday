...
func TestLauncher_ResumeArgUsesSessionFlag(t *testing.T) {
	...
	want := []string{"run", "-s", "ses_abc123", "--auto", "do thing"}
...
// An unset AgentName means opencode's own default agent runs; mouse.yaml can
// name one explicitly.
func TestLauncher_AgentNameDefaultAndOverride(t *testing.T) {
	cfg := SpawnConfig{Dir: t.TempDir(), Binary: "opencode", Task: "do thing"}

	_, args := buildArgs(cfg)
	if !reflect.DeepEqual(args, []string{"run", "--auto", "do thing"}) {
